...
class ExpansionTests(unittest.TestCase):
 ...
 def test_explicit_evidence_date_overrides_later_retrieval(self):
  ...
  self.assertIsNone(n.overlap(a,b))
 def test_public_audit_allows_retrieval_after_independent_evidence(self):
  data=json.loads((ROOT/'data/atlas.json').read_text())
  post=next(p for p in data['career_posts'] if p.get('is_current') and p.get('start') and p.get('as_of_date') and not p.get('end'))
  post['checked_at']='2026-09-21'
  post['as_of_date']='2026-09-16'
  audit_data.audit(data)
  original=n.interval
  def unsupported_boundary(p):
   result=original(p)
   return (result[0],n.date_bounds(p['checked_at'])) if result and p['id']==post['id'] else result
  with patch.object(audit_data,'interval',unsupported_boundary):
   with self.assertRaisesRegex(AssertionError,'independent evidence'):audit_data.audit(data)
