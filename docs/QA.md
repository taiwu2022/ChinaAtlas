...
## Global search and concise profiles (2026-09-21)

- A shared public-field index now searches every stored identity and institution from every page, independent of previously selected region/department/status filters. Typeahead is also used for network centers and pair comparisons. Stable IDs, distinct homonyms, aliases and IME composition are covered by 16 dedicated search checks. A production-render regression checks all six routes and restoration after clearing search; navigation now has 16 checks.
- Compact profiles omit empty event/history sections, unverified-rank placeholders and routine editorial text. Duplicate role/career records require a unique compatible match in both directions; ambiguous undated terms remain separate. Important conflicts, unresolved identities, pending disciplinary procedures and evidence dates stay visible. Full source metadata is retained behind disclosures; reading presentation does not mutate any record. Tests render all 697 identities and cover repeated terms, undated matching, escaping and private note selectors.
- In-browser checks exercised actual Liu Ruifeng lookup from the overview, network center selection, Zhang Haibo homonym suggestions and explicit pair comparison, plus PBOC alias lookup from the reading page and the no-result state. Profile and global search geometry fit 320/390/1280 px; mobile profile and comparison picker, and desktop profile were visually inspected. These are emulated viewports.
- 117 public and 105 maintenance Python tests pass; frontend, personnel, portrait, storage, navigation, search and profile reading suites pass. Local frontend passes. Maintenance HTTP serves the same new modules; full SQLite rows equal the backup (30 records, 1 review, 2 revisions, 0 profile notes). Public data/evidence/portrait snapshots are unchanged in this release.
