...
function publicRegionExport(id){
 ...
 copy.profile_facts=(copy.profile_facts||[]).filter(f=>pids.has(f.person_id));
...
function drawPublicChanges(){const events=atlas.events.filter(e=>e.status==='verified'&&match(e)).sort((a,b)=>b.date.localeCompare(a.date));return `<div class="section-title"><p class="note">已核对的任免 · ${events.length} 条</p>${external('https://www.12371.cn/special/rsrm/','查看官方新公告')}</div><p class="note compact-note">截至 ${esc(atlas.verified_at)} 的资料快照；新公告核实后再更新人物。</p>${events.map(eventCard).join('')||'<p class="empty">没有匹配的任免。</p>'}`;}
