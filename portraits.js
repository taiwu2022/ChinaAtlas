...
function profileHeading(p){return `<div class="profile-heading"><div class="profile-identity"><span class="pill">${current(p).length?'现任与历任职务':'历史人物'}</span><h2 class="detail-title">${esc(p.name)}</h2><p class="detail-subtitle"><span lang="en">${esc(p.name_en||'')}${p.birth_year?' · '+esc(p.birth_year)+'年生':''}</span></p></div><div class="portrait-slot" data-portrait-for="${esc(p.id)}">${portraitHTML(p)}</div></div><p class="profile-checked">资料核对截至 ${esc(p.checked_at)}</p>`;}
