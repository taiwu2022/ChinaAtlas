...
def build():
 ...
 shutil.copyfile(ROOT/'data/portraits.json',target/'portraits.json')
 portrait_digest=hashlib.sha256((target/'portraits.json').read_bytes()).hexdigest()[:12]
 digest=hashlib.sha256((target/'data/atlas.json').read_bytes()).hexdigest()[:12]
 (target/'site-config.js').write_text('window.ATLAS_CONFIG='+json.dumps({'mode':'public','dataVersion':digest,'portraitVersion':portrait_digest})+';\n')
 ...
 (target/'build-info.json').write_text(json.dumps({'source_commit':revision,'data_version':digest,'portrait_version':portrait_digest})+'\n')
 output=ROOT/'site'
