...
- Shared search.js indexes explicit public names, office titles, institutions and places across all routes; never index personal notes or raw audit metadata. Keep stable-ID selection, homonym disambiguation and IME-safe keyboard handling. Profiles use profile.js: hide empty sections and routine editorial caveats, consolidate duplicate office records only with compatible dates/status, preserve substantive uncertainty/status events and all source metadata in disclosures. Mirror both new modules into the maintenance asset allowlist. Run search.cjs and profile-reading.cjs alongside navigation/storage tests.
